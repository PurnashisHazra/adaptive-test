...
import {
  ...
  getTestSubjects,
  getTestTopics,
  listChallengeAssignments,
  listQuestions,
  listStudentUsernames,
  syncChallengeAssignments,
  updateChallenge,
} from "../../api/client";
import type { AppConfig, ChallengeLevel, Difficulty, ExamTag, QuestionAdmin, QuestionPaperSection } from "../../api/types";
import {
  datetimeLocalInputToUtcIso,
  defaultChallengeScheduleInputs,
  formatNowIST,
  previewDatetimeLocalIST,
  toDatetimeLocalInputIST,
} from "../../lib/istTime";

const LEVEL_OPTIONS: ChallengeLevel[] = ["BEGINNER", "INTERMEDIATE", "ADVANCED", "EXPERT"];
...
export function ChallengeFormPage() {
  ...
  const defaults = defaultChallengeScheduleInputs();

  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [level, setLevel] = useState<ChallengeLevel>("INTERMEDIATE");
  const [isAdaptive, setIsAdaptive] = useState(true);
  const [launchAt, setLaunchAt] = useState(defaults.launch);
  const [endAt, setEndAt] = useState(defaults.end);
  const [openToAll, setOpenToAll] = useState(false);

  const launchPreview = useMemo(() => previewDatetimeLocalIST(launchAt), [launchAt]);
  const endPreview = useMemo(() => previewDatetimeLocalIST(endAt), [endAt]);
  ...
  useEffect(() => {
    ...
    getChallenge(id)
      .then((p) => {
        ...
        setLevel(p.level);
        setIsAdaptive(p.is_adaptive);
        setLaunchAt(toDatetimeLocalInputIST(p.launch_at));
        setEndAt(toDatetimeLocalInputIST(p.end_at));
        setOpenToAll(p.open_to_all);
        ...
        return listChallengeAssignments(id);
      })
      .then(setAssignments)
  ...
  async function onSave(e: React.FormEvent) {
    ...
    let launchIso: string;
    let endIso: string;
    try {
      launchIso = datetimeLocalInputToUtcIso(launchAt);
      endIso = datetimeLocalInputToUtcIso(endAt);
    } catch {
      toast.error("Invalid schedule dates");
      return;
    }
    if (new Date(endIso) <= new Date(launchIso)) {
      toast.error("End time must be after launch time");
      return;
    }
    const payload = {
      title: title.trim(),
    ...
    try {
      if (isNew) {
        const p = await createChallenge(payload);
        await syncChallengeAssignments(p.id, Array.from(checkedStudents));
        toast.success("Challenge created");
        nav(`/admin/challenges/${p.id}`, { replace: true });
      } else if (id) {
  ...
  async function onSaveAssignments() {
    ...
    try {
      await syncChallengeAssignments(id, Array.from(checkedStudents));
      toast.success("Assignments saved");
      setAssignments(await listChallengeAssignments(id));
    } catch (err: unknown) {
      const msg =
        err && typeof err === "object" && "response" in err
          ? (err as { response?: { data?: { detail?: string } } }).response?.data?.detail
  ...
  return (
    <AdminPanel
      ...
      actions={
        <Link to="/admin/challenges" className="btn btn-ghost">
          ← All challenges
        </Link>
      }
    >
      <form onSubmit={onSave} className="card" style={{ maxWidth: 720 }}>
        ...
        <div style={{ marginBottom: "1rem" }}>
          ...
          <textarea className="input" rows={3} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Shown on the home page" />
        </div>
        <div className="grid-2" style={{ marginBottom: "1rem" }}>
          <div>
            <label className="label">Level</label>
            <select className="input" value={level} onChange={(e) => setLevel(e.target.value as ChallengeLevel)}>
              {LEVEL_OPTIONS.map((l) => (
          ...
          <div>
            ...
            <select className="input" value={isAdaptive ? "adaptive" : "fixed"} onChange={(e) => setIsAdaptive(e.target.value === "adaptive")}>
              <option value="adaptive">Adaptive</option>
              <option value="fixed">Non-adaptive (fixed set)</option>
            </select>
          </div>
        </div>
        <p style={{ color: "var(--muted)", fontSize: "0.9rem", margin: "0 0 0.75rem" }}>
          Schedule times are <strong>Indian Standard Time (IST)</strong>. Now: {formatNowIST()}. The picker shows IST
          wall clock (not your browser timezone).
        </p>
        <div className="grid-2" style={{ marginBottom: "1rem" }}>
          <div>
            <label className="label">Launch (IST)</label>
            <input className="input" type="datetime-local" value={launchAt} onChange={(e) => setLaunchAt(e.target.value)} required />
            {launchPreview ? (
              <p style={{ margin: "0.35rem 0 0", fontSize: "0.85rem", color: "var(--muted)" }}>Saves as {launchPreview}</p>
            ) : null}
          </div>
          <div>
            <label className="label">End (IST)</label>
            <input className="input" type="datetime-local" value={endAt} onChange={(e) => setEndAt(e.target.value)} required />
            {endPreview ? (
              <p style={{ margin: "0.35rem 0 0", fontSize: "0.85rem", color: "var(--muted)" }}>Saves as {endPreview}</p>
            ) : null}
          </div>
        </div>
        <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "1rem", cursor: "pointer" }}>
          <input type="checkbox" checked={openToAll} onChange={(e) => setOpenToAll(e.target.checked)} />
          <span>Open to all students on the platform (no assignment required)</span>
        </label>
        <div className="grid-2" style={{ marginBottom: "1rem" }}>
          <div>
      ...
      {!isNew && id ? (
        <div className="card" style={{ marginTop: "1.25rem" }}>
          ...
          <p style={{ color: "var(--muted)", fontSize: "0.9rem" }}>
            Assign students who can enter when &quot;open to all&quot; is off. Unchecked students lose access to this challenge only.
          </p>
