...
function AdaptiveTestPitchBody({
  showCta,
  signedIn,
}: {
  showCta: boolean;
...
export function AdaptiveTestPitch({
  ...
  showCta?: boolean;
  signedIn?: boolean;
  /** Show full content without the Learn more toggle (e.g. inside a modal). */
  expanded?: boolean;
  className?: string;
}) {
  const rootClass = ["adaptive-pitch", expanded ? "adaptive-pitch--expanded" : "", className].filter(Boolean).join(" ");

  return (
    <section className={rootClass} aria-labelledby="adaptive-pitch-heading">
      ...
      {expanded ? (
        <div className="adaptive-pitch__more-body">
          <AdaptiveTestPitchBody showCta={showCta} signedIn={signedIn} />
        </div>
      ) : (
